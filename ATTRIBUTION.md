...
- **Per-session chat isolation:** Replaced a global single-chain pattern (which silently shared one session across all listeners) with a `Map<jamSessionId+djId, chat>` so each DJ in each jam keeps its own persistent context.
